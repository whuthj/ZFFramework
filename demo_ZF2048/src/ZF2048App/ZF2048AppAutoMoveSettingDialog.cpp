...
public:
    ...
    virtual void objectOnInitFinish(void)
    {
        ...
        this->actionList()->listCellSizeHintSet(-1);
        ...
        this->actionList()->listUpdater()->add(zflineAlloc(ZFUIListCellUpdaterBasic));
...
public:
    static ZFLISTENER_PROTOTYPE_EXPAND(addActionOnClick)
    {
        ...
        owner->d->actionListAdapter()->listCellAdd(cell);
        ...
        owner->d->confirmButton()->buttonEnableSet(owner->d->actionListAdapter()->listCellCount() > 0);
    }
    static ZFLISTENER_PROTOTYPE_EXPAND(actionItemOnClick)
    {
        ...
        zfindex index = owner->d->actionListAdapter()->listCellArray()->find(actionItem->viewParent(), ZFComparerCheckEqualOnly);
        zfCoreAssert(index != zfindexMax);
        owner->autoMoves.remove(index);
        owner->d->actionListAdapter()->listCellRemove(index);
        owner->d->actionList()->listReload();
