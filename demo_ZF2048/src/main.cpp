...
{
    ZFGlobalEventCenter::instance()->observerAdd(ZFUIView::EventViewOnEvent(), ZFCallbackForRawFunction(zfself::action));
}
ZF_GLOBAL_INITIALIZER_DESTROY(testLangLoad)
{
    ZFGlobalEventCenter::instance()->observerRemove(ZFUIView::EventViewOnEvent(), ZFCallbackForRawFunction(zfself::action));
}
