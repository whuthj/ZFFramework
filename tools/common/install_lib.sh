...
if test "x$PROJ_NAME" = "x" || test "x$PROJ_DIR_NAME" = "x" || test "x$SRC_PATH" = "x" || test "x$DST_PATH" = "x" ; then
    echo usage:
    echo   install_lib.sh PROJ_NAME PROJ_DIR_NAME SRC_PATH DST_PATH
    exit 1
fi

mkdir -p "$DST_PATH" >/dev/null 2>&1
find "$SRC_PATH" -regex ".*/.*\.dll" 2>&1 | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>&1
find "$SRC_PATH" -regex ".*/.*\.a" 2>&1 | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>&1
find "$SRC_PATH" -regex ".*/.*\.so" 2>&1 | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>&1
find "$SRC_PATH" -regex ".*/.*\.dylib" 2>&1 | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>&1

for line in $(find "$DST_PATH" -type f -name "*.a" 2>&1); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
...
for line in $(find "$DST_PATH" -type f -name "*.so" 2>&1); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
...
for line in $(find "$DST_PATH" -type f -name "*.dylib" 2>&1); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
