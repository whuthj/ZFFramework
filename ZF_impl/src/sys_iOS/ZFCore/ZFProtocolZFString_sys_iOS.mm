...
#include "../../thirdparty/UTFCodeUtil/UTFCodeUtil.h"
...
ZFPROTOCOL_IMPLEMENTATION_BEGIN(ZFStringImpl_sys_iOS, ZFString, ZFProtocolLevelSystemNormal)
...
public:
    ...
    {
        return [ZFCastStatic(NSString *, nativeString) retain];
    }
    virtual void nativeStringRelease(ZF_IN void *nativeString)
    {
        [ZFCastStatic(NSString *, nativeString) release];
    }
                                   ...
                                   ZF_OUT void *&stringValueToken)
    {
        ...
        zfchar *tmp = zfsCopy(ZFCastStatic(NSString *, nativeString).UTF8String);
        stringValue = tmp;
