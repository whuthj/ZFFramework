...
#include "ZFAlgorithm.h" // for ZFRegExp
...
static void ZFEnvInfoImpl_default_getVersion(ZF_IN_OUT zfstring &ret, ZF_IN const zfchar *src)
{
    ...
    {
        ret.append(src + result.namedGroups[0].start, result.namedGroups[0].count);
    }
}

ZFPROTOCOL_IMPLEMENTATION_BEGIN(ZFEnvInfo_systemInfoImpl_default, ZFEnvInfo_systemInfo, ZFProtocolLevelDefault)
public:
    ...
    virtual void systemVersion(ZF_IN_OUT zfstring &ret)
    {
        ...
        #if ZF_ENV_sys_Posix
        {
            ...
            zfmemset(&buf, 0, sizeof(struct utsname));
            uname(&buf);
            ZFEnvInfoImpl_default_getVersion(ret, zfsCoreA2Z(buf.version));
            return ;
        }
        #endif
    }
...
ZFPROTOCOL_IMPLEMENTATION_BEGIN(ZFEnvInfo_frameworkInfoImpl_default, ZFEnvInfo_frameworkInfo, ZFProtocolLevelDefault)
public:
