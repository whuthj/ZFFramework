...
public:
    /** @cond ZFPrivateDoc */
    inline ZFCoreString_impl &operator = (ZF_IN const ZFCoreString_impl &s) {return this->assign(s);}
...
public:
    ...
    void remove(ZF_IN zfindex pos = 0, ZF_IN zfindex len = zfindexMax)
    {
        if(pos < d.length)
        {
            ...
            if(len > 0)
            {
                T_Char *buf = (d.builtinBuf ? d.d.buf : d.d.s.s);
...
public:
    ...
    {
        const T_Char *buf = (d.builtinBuf ? d.d.buf : d.d.s.s);
        if(s)
        {
            if(len == zfindexMax)
            {
