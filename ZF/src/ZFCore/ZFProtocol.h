 ...
 */
typedef enum {
    ZFProtocolLevelDefault = 0, /**< @brief see #ZFPROTOCOL_IMPLEMENTATION_BEGIN */
    ZFProtocolLevelSystemLow = 1, /**< @brief see #ZFPROTOCOL_IMPLEMENTATION_BEGIN */
    ZFProtocolLevelSystemNormal = 2, /**< @brief see #ZFPROTOCOL_IMPLEMENTATION_BEGIN */
    ZFProtocolLevelSystemHigh = 3, /**< @brief see #ZFPROTOCOL_IMPLEMENTATION_BEGIN */
    ZFProtocolLevelApp = 4, /**< @brief see #ZFPROTOCOL_IMPLEMENTATION_BEGIN */
} ZFProtocolLevel;

/** @brief string tokens */
#define ZFTOKEN_ZFProtocolLevelDefault zfText("LevelDefault")
/** @brief string tokens */
#define ZFTOKEN_ZFProtocolLevelSystemLow zfText("LevelSystemLow")
/** @brief string tokens */
#define ZFTOKEN_ZFProtocolLevelSystemNormal zfText("LevelSystemNormal")
/** @brief string tokens */
#define ZFTOKEN_ZFProtocolLevelSystemHigh zfText("LevelSystemHigh")
/** @brief string tokens */
#define ZFTOKEN_ZFProtocolLevelApp zfText("LevelApp")

ZFCORETYPE_STRING_CONVERTER_DECLARE(ZFProtocolLevel, ZFProtocolLevel)
...
public:
     ...
     */
    virtual ZFProtocolLevel protocolImplementationLevel(void) const
    {
        return ZFProtocolLevelDefault;
    }
    /**
...
public:
    ...
    virtual void objectInfoT(ZF_IN_OUT zfstring &ret) const
    {
        zfstringAppend(ret, zfText("<[%s] %s%s>"),
            ...
            (!zfscmpTheSame(this->protocolImplementationPlatformHint(), zfText("")) ? zfstringWithFormat(zfText(" (%s)"), this->protocolImplementationPlatformHint()).cString() : zfText(""))
            );
    }
        ...
        public: \
            ...
            { \
                ...
                { \
                    ...
                    { \
                        zfCoreCriticalMessageTrim(zfTextA("only one implementation is allowed for protocol \"%s\", while we have: \"%s\", \"%s\""), \
                            #ModuleName, \
                            _d.implName.cString(), \
                            implName); \
                        return ; \
                    ...
                    { \
                        /* \
                        zfCoreLogTrim(zfTextA("ignore implementation \"%s\" while having \"%s\" for protocol \"%s\""), \
                    ...
                    { \
                        /* \
                        zfCoreLogTrim(zfTextA("reset implementation for protocol \"%s\", old: \"%s\", new: \"%s\""), \
            ...
            { \
                ...
                if(implConstructor != zfnull && !zfscmpTheSame(implName, zfText(""))) \
                { \
                    _d.implConstructor = implConstructor; \
                    _d.implName = implName; \
                    _d.implLevel = implLevel; \
                ...
                { \
                    ...
                    _d.implLevel = ZFProtocolLevelDefault; \
                } \
            } \
            ...
            { \
                ...
                { \
                    ...
                    { \
                        ZFFrameworkAssertInit(); \
                        zfCoreCriticalMessageTrim(zfTextA("no implementation for protocol \"%s\""), \
                            ZFMACRO_TOSTRING_A(ModuleName)); \
                        return zfnull; \
                    } \
                    ...
                    { \
                        ...
                        zfCoreLogTrimNoEndl(zfTextA("(%s)%s depends on (%s)\"%s\""), \
                            zfsCoreZ2A(_d.implInstance->protocolName()), \
                            zfsCoreZ2A(_d.implInstance->protocolImplementationName()), \
                            zfsCoreZ2A(mismatchProtocolName), \
                            zfsCoreZ2A(desiredImplPlatformHint)); \
                        if(mismatchImpl == zfnull) \
                        { \
                            zfCoreLogTrim(zfTextA(" but it's not implemented")); \
                        } \
                        else \
                        { \
                            zfCoreLogTrim(zfTextA(" but it's implementation \"%s\" mismatch"), \
                                zfsCoreZ2A(mismatchImpl->protocolImplementationPlatformHint())); \
                        } \
                        zfCoreCriticalError(); \
...
#define ZFPROTOCOL_INTERFACE_REGISTER(ModuleName) \
    ZF_STATIC_INITIALIZER_INIT_WITH_LEVEL(ZFProtocolInterfaceRegister_##ModuleName, ZFLevelZFFrameworkEssential) \
    { \
...
#define _ZFP_ZFPROTOCOL_IMPLEMENTATION_BEGIN_(ImplementationName, ImplementationClass, ModuleName, implLevel) \
    ...
    { \
        ...
        public: \
            ...
            virtual const zfchar *protocolImplementationName(void) const \
            { \
                return zfText(#ImplementationName); \
            } \
            zfoverride \
            virtual ZFProtocolLevel protocolImplementationLevel(void) const \
            { \
        ...
        public: \
            ...
            { \
                ...
                { \
                    ZFPROTOCOL_INTERFACE_CLASS(ModuleName)::_ZFP_ZFProtocolImplementationChange(zfnull, zfnull, ZFProtocolLevelDefault); \
                } \
                _ZFP_ZFProtocolImplDataUnregister(zfself::_ZFP_ZFProtocolZFCoreLibDestroyFlag(), zfTextA(#ModuleName)); \
            } \
...
#define _ZFP_ZFPROTOCOL_IMPLEMENTATION_REGISTER_(ImplementationName, ImplementationClass) \
    ZF_STATIC_INITIALIZER_INIT_WITH_LEVEL(ZFProtocolImplRegister_##ImplementationClass, ZFLevelZFFrameworkEssential) \
    { \
 ...
 *   and in your wrapper object, supply a native method named "nativeImpl"
 ...
 *           ZFPROTOCOL_IMPLEMENTATION_PLATFORM_DEPENDENCY_ITEM(ZFString, zfText("NSString))
...
#define ZFPROTOCOL_IMPLEMENTATION_PLATFORM_DEPENDENCY_ITEM(Protocol, DependencyPlatformHint) \
            ...
            { \
                ...
                mismatchProtocolName = ZFMACRO_TOSTRING(Protocol); \
 ...
 *       public: // you can add constructors, but with no arguments only
 *           ZFPROTOCOL_IMPLEMENTATION_CLASS(YourImplementation)(void) {}
 ...
 * -  ZFProtocolLevelDefault
 * -  ZFProtocolLevelSystemLow
 * -  ZFProtocolLevelSystemNormal
 * -  ZFProtocolLevelSystemHigh
 * -  ZFProtocolLevelApp
 *
