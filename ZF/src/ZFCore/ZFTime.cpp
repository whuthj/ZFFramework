...
ZFPROPERTY_TYPE_INFO_GETTER_REGISTER(ZFTimeValue, ZFTimeValue)
...
zfbool ZFTime::serializableOnSerializeCategoryFromData(ZF_IN const ZFSerializableData &ownerData,
                                                       ...
                                                       ZF_OUT_OPT const ZFSerializableData **outErrorPos /* = zfnull */)
{
    ...
    { // timeValue
        const ZFSerializableData *categoryData = ZFSerializableUtil::checkElementByCategory(ownerData, ZFSerializableKeyword_ZFTime_timeValue);
        if(categoryData != zfnull)
        {
            ZFTimeValue timeValue = ZFTimeValueZero;
            if(!ZFTimeValueFromSerializableData(timeValue, *categoryData, outErrorHintToAppend, outErrorPos))
            {
                return zffalse;
            }
            ...
            {
                this->serializableRefInfoSetForCategory(ZFSerializableKeyword_ZFTime_timeValue, categoryData->referenceInfoCopy());
            }
        }
        else
        {
            this->timeValueSet(ZFTimeValueZero);
        }
    }

    { // timeZone
        const ZFSerializableData *categoryData = ZFSerializableUtil::checkElementByCategory(ownerData, ZFSerializableKeyword_ZFTime_timeZone);
        if(categoryData != zfnull)
        {
            ZFTimeValue timeZone = ZFTimeValueZero;
            if(!ZFTimeValueFromSerializableData(timeZone, *categoryData, outErrorHintToAppend, outErrorPos))
            {
                return zffalse;
            }
            ...
            if(categoryData->referenceInfoExist())
            {
                this->serializableRefInfoSetForCategory(ZFSerializableKeyword_ZFTime_timeZone, categoryData->referenceInfoCopy());
            }
        }
        else
        {
            this->timeZoneSet(ZFTimeValueZero);
        }
    }
    return zftrue;
}
zfbool ZFTime::serializableOnSerializeCategoryToData(ZF_IN_OUT ZFSerializableData &ownerData,
                                                     ...
                                                     ZF_OUT_OPT zfstring *outErrorHintToAppend /* = zfnull */)
{
    ...
    { // timeValue
        ...
        if(!ZFTimeValueToSerializableData(categoryData, this->timeValue(), outErrorHintToAppend))
        {
            return zffalse;
        }
        categoryData.categorySet(ZFSerializableKeyword_ZFTime_timeValue);

        zfbool needAdd = zftrue;
        if(ref != zfnull)
        {
            if(ref->timeValue() == this->timeValue())
            {
                needAdd = zffalse;
            }
        }
        if(needAdd)
        {
            const ZFSerializableData *refInfo = this->serializableRefInfoGetForCategory(ZFSerializableKeyword_ZFTime_timeValue);
            if(refInfo != zfnull)
            {
            ...
            else if(this->timeValue() == ZFTimeValueZero)
            {
                needAdd = zffalse;
            }
        }
    ...
    { // timeZone
        ZFSerializableData categoryData;
        if(!ZFTimeValueToSerializableData(categoryData, this->timeZone(), outErrorHintToAppend))
        {
            return zffalse;
        }
        categoryData.categorySet(ZFSerializableKeyword_ZFTime_timeZone);

        zfbool needAdd = zftrue;
        if(ref != zfnull)
        {
            if(ref->timeZone() == this->timeZone())
            {
                needAdd = zffalse;
            }
        }
        if(needAdd)
        {
            const ZFSerializableData *refInfo = this->serializableRefInfoGetForCategory(ZFSerializableKeyword_ZFTime_timeZone);
            if(refInfo != zfnull)
            {
                ZFSerializableData tmpData;
                if(!refInfo->referenceLoadTo(tmpData, outErrorHintToAppend))
            ...
            else if(this->timeZone() == ZFTimeValueZero)
            {
                needAdd = zffalse;
            }
        }
