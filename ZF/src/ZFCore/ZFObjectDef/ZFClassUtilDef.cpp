...
ZFCoreArrayPOD<const ZFClass *> allClass(ZF_IN const ZFClass *cls,
...
ZFCoreArrayPOD<const ZFMethod *> allMethod(ZF_IN const ZFClass *cls,
                                           ZF_IN_OPT const ZFMethodFilter *filter /* = zfnull */)
{
    ZFCoreArrayPOD<const ZFMethod *> ret;
    ZFCoreArrayPOD<const ZFClass *> allClasses = ZFClassUtil::allClass(cls, (filter == zfnull) ? zfnull : &(filter->classFilter));
    for(zfindex i = 0; i < allClasses.count(); ++i)
    {
        const ZFClass *cls = allClasses.get(i);
...
ZFCoreArrayPOD<const ZFProperty *> allProperty(ZF_IN const ZFClass *cls,
