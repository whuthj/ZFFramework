...
// ============================================================
ZF_GLOBAL_INITIALIZER_INIT_WITH_LEVEL(ZFPropertyInfoGetterHolder, ZFLevelZFFrameworkNormal)
{
}
ZFCoreMap propertyTypeInfoGetterMap; // ZFPropertyTypeInfoGetterCallback
ZF_GLOBAL_INITIALIZER_END(ZFPropertyInfoGetterHolder)
#define _ZFP_ZFPropertyInfoGetterMap (ZF_GLOBAL_INITIALIZER_INSTANCE(ZFPropertyInfoGetterHolder)->propertyTypeInfoGetterMap)

// ============================================================
void ZFPropertyTypeInfoGetterRegister(ZF_IN const zfchar *typeIdName,
                                      ZF_IN ZFPropertyTypeInfoGetterCallback propertyTypeInfoGetterCallback)
{
    zfCoreAssert(typeIdName != zfnull);
    _ZFP_ZFPropertyInfoGetterMap.set(typeIdName, ZFCorePointerForObject<ZFPropertyTypeInfoGetterCallback *>(zfnew(ZFPropertyTypeInfoGetterCallback, propertyTypeInfoGetterCallback)));
}
void ZFPropertyTypeInfoGetterUnregister(ZF_IN const zfchar *typeIdName)
{
    _ZFP_ZFPropertyInfoGetterMap.remove(typeIdName);
}

ZFPropertyTypeInfoGetterCallback ZFPropertyTypeGetInfoGetterCallback(ZF_IN const zfchar *typeIdName)
{
    ZFPropertyTypeInfoGetterCallback *propertyTypeInfoGetterTmp = _ZFP_ZFPropertyInfoGetterMap.get<ZFPropertyTypeInfoGetterCallback *>(typeIdName);
    return ((propertyTypeInfoGetterTmp != zfnull) ? (*propertyTypeInfoGetterTmp) : zfnull);
}
void ZFPropertyGetInfo(ZF_IN_OUT zfstring &ret,
                       ZF_IN const ZFProperty *propertyInfo,
                       ZF_IN ZFObject *ownerObject)
{
#define _ZFP_ZFPropertyGetInfo_NA zfText("NA")
    if(ownerObject == zfnull || propertyInfo == zfnull)
    {
        ret += _ZFP_ZFPropertyGetInfo_NA;
        return ;
    }
    if(propertyInfo->propertyIsRetainProperty())
    {
        ZFobjectInfoT(ret, propertyInfo->callbackRetainGet(propertyInfo, ownerObject));
        return ;
    }

    if(zfscmpTheSame(propertyInfo->propertyTypeIdName(), ZFPropertyTypeId_none))
    {
        ret += _ZFP_ZFPropertyGetInfo_NA;
        return ;
    }

    ZFPropertyTypeInfoGetterCallback propertyTypeInfoGetterCallback = ZFPropertyTypeGetInfoGetterCallback(propertyInfo->propertyTypeIdName());
    if(propertyTypeInfoGetterCallback == zfnull)
    {
        ret += _ZFP_ZFPropertyGetInfo_NA;
        return ;
    }
    propertyTypeInfoGetterCallback(ret, propertyInfo, ownerObject);
}
