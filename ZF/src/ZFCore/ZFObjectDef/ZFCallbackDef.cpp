...
// ============================================================
// global
ZF_STATIC_INITIALIZER_INIT_WITH_LEVEL(ZFCallbackDataHolder, ZFLevelZFFrameworkEssential)
{
                                             ...
                                             ZF_IN zfindex createInfoFileLine)
{
    ...
    switch(callbackType)
    {
        ...
        case ZFCallbackTypeMethod:
            ...
            zfCoreAssertWithMessageTrim(callbackMethod->methodType() == ZFMethodTypeClassStatic || callbackMethod->methodType() == ZFMethodTypeRawFunction,
                zfTextA("%s method \"%s\" not function type or class static member type"),
                zfsCoreZ2A(callback.createInfo().cString()),
        ...
        case ZFCallbackTypeMemberMethod:
            ...
            zfCoreAssertWithMessageTrim(callbackMethod->methodType() == ZFMethodTypeClassMember,
                zfTextA("%s method \"%s\" not class member type"),
                zfsCoreZ2A(callback.createInfo().cString()),
                zfsCoreZ2A(callbackMethod->objectInfo().cString()));
...
void ZFCallback::callbackSerializableOwnerTagSet(ZF_IN const zfchar *ownerTag)
{
    zfsChange(d->serializableOwnerTag, ownerTag);
}
const zfchar *ZFCallback::callbackSerializableOwnerTag(void) const
{
    return d->serializableOwnerTag;
}
...
void ZFCallback::_ZFP_ZFCallbackCachedDataSetup(ZF_IN_OUT ZFCallback &c, _ZFP_ZFCallbackPrivate *d)
{
    switch(d->callbackType)
    {
        case ZFCallbackTypeDummy:
            ...
            c._ZFP_ZFCallbackCached_callbackInvoker = zfnull;
            c._ZFP_ZFCallbackCached_callbackOwnerObj = zfnull;
            break;
        case ZFCallbackTypeMethod:
            switch(d->callbackMethod->methodType())
            {
                ...
                case ZFMethodTypeClassMember:
                    ...
                    c._ZFP_ZFCallbackCached_callbackInvoker = d->callbackMethod->methodInvokerFunctionAddress();
                ...
                case ZFMethodTypeRawFunction:
                    ...
                    c._ZFP_ZFCallbackCached_callbackInvoker = d->callbackMethod->methodFunctionAddress();
                    c._ZFP_ZFCallbackCached_callbackOwnerObj = zfnull;
                    break;
                default:
                    zfCoreCriticalShouldNotGoHere();
        ...
        case ZFCallbackTypeRawFunction:
            c._ZFP_ZFCallbackCached_callbackType = _ZFP_ZFCallbackCachedTypeRawFunction;
            c._ZFP_ZFCallbackCached_callbackInvoker = d->callbackRawFunc;
            c._ZFP_ZFCallbackCached_callbackOwnerObj = zfnull;
            break;
        default:
            zfCoreCriticalShouldNotGoHere();
