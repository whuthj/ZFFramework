...
{
    ZFCoreArrayPOD<ZFClass *> allClassDelayed = this->delayDeleteMap.allValue<ZFClass *>();
    this->delayDeleteMap.removeAll();
    ZFCoreArrayPOD<ZFClass *> allClass = this->classMap.allValue<ZFClass *>();
...
public:
    ...
    {
        const ZFClass *cls = this->ownerClass;
...
{
    ZFCoreArrayPOD<const ZFClass *> allCls = _ZFP_ZFClassMap.allValue<const ZFClass *>();
    for(zfindex i = 0; i < allCls.count(); ++i)
    {
        allCls.get(i)->classTagRemoveAll();
    }
}
                                                         ...
                                                         ...)
{
    ...
    d->interfaceCastCallbackListCache = (_ZFP_ZFObjectToInterfaceCastCallback *)zfrealloc(d->interfaceCastCallbackListCache, sizeof(_ZFP_ZFObjectToInterfaceCastCallback) * (parentInterfaceCastList.count() + 1));
...
{ // copy parent's interface cast datas
    ...
    cls->d->interfaceCastCallbackListCache = (_ZFP_ZFObjectToInterfaceCastCallback *)zfrealloc(cls->d->interfaceCastCallbackListCache, sizeof(_ZFP_ZFObjectToInterfaceCastCallback) * (parentInterfaceCastList.count() + 1));
...
void ZFClass::_ZFP_ZFClass_propertyRegister(ZF_IN const ZFProperty *zfproperty)
{
    d->propertyMap.set(zfproperty->propertyName(), ZFCorePointerForObject<const ZFProperty **>(zfnew(const ZFProperty *, zfproperty)));
