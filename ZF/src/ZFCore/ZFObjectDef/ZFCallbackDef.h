...
typedef enum
{
    ...
    ZFCallbackTypeMethod, /**< @brief class static member method or static function described by #ZFMethod */
...
ZFCOMPARER_DEFAULT_DECLARE_BEGIN(ZFCallbackType, e0, ZFCallbackType, e1)
{
    return ((e0 == e1) ? ZFCompareTheSame : ZFCompareUncomparable);
}
ZFCOMPARER_DEFAULT_DECLARE_END(ZFCallbackType, e0, ZFCallbackType, e1)
...
#define _ZFP_ZFCALLBACK_INVOKER(N) \
    ...
    T_ReturnType executeExact(ZFMACRO_REPEAT(N, ZFMACRO_REPEAT_PARAM, ZFMACRO_EMPTY)) const \
    { \
        ...
        { \
            ...
            case _ZFP_ZFCallbackCachedTypeClassMember: \
                return ((T_ReturnType (*)(ZFObject * ZFMACRO_REPEAT(N, ZFMACRO_REPEAT_TYPE, ZFMACRO_COMMA)))(_ZFP_ZFCallbackCached_callbackInvoker)) \
                    (_ZFP_ZFCallbackCached_callbackOwnerObj ZFMACRO_REPEAT(N, ZFMACRO_REPEAT_NAME, ZFMACRO_COMMA)); \
            case _ZFP_ZFCallbackCachedTypeRawFunction: \
                return ((T_ReturnType (*)(ZFMACRO_REPEAT(N, ZFMACRO_REPEAT_TYPE, ZFMACRO_EMPTY)))(_ZFP_ZFCallbackCached_callbackInvoker)) \
                    (ZFMACRO_REPEAT(N, ZFMACRO_REPEAT_NAME, ZFMACRO_EMPTY)); \
            default: \
                break; \
        } \
        /* typically should not go here */ \
        return ((T_ReturnType (*)(void))(_ZFP_ZFCallbackCached_callbackInvoker)) (); \
    }
 ...
 *
 * a callback may be a class member or static member,
 * or static function declared as #ZFMethod,
 ...
 * to execute callback, use #executeExact similar to ZFMethod::executeFunction,
...
public:
     ...
     */
    zffinal void callbackSerializableOwnerTagSet(ZF_IN const zfchar *ownerTag);
    /**
     * @brief see #ZFPropertyTypeId_ZFCallback
     */
    zffinal const zfchar *callbackSerializableOwnerTag(void) const;
    /**
     * @brief see #ZFPropertyTypeId_ZFCallback
     */
...
private:
    ...
    ZFFuncAddrType _ZFP_ZFCallbackCached_callbackInvoker;
    ZFObject *_ZFP_ZFCallbackCached_callbackOwnerObj;
};

ZFCOMPARER_DEFAULT_DECLARE_BEGIN(ZFCallback, e0, ZFCallback, e1)
{
    return ((e0 == e1) ? ZFCompareTheSame : ZFCompareUncomparable);
}
ZFCOMPARER_DEFAULT_DECLARE_END(ZFCallback, e0, ZFCallback, e1)
...
#define ZFCALLBACK_LOCAL_BEGIN_0(ReturnType, callbackName) \
    class ZFMACRO_CAT(_ZFP_ZFCallbackLocal_, callbackName) \
    { \
    public: \
...
#define ZFCALLBACK_LOCAL_END_WITH_TYPE(CallbackType, callbackName) \
    ...
    CallbackType callbackName = ZFCallbackForRawFunction((ZFFuncAddrType)(ZFMACRO_CAT(_ZFP_ZFCallbackLocal_, callbackName)::callbackName));
