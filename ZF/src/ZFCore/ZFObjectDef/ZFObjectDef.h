...
#include "ZFObjectInstanceObserverDef.h"
...
public:
     ...
     *
     * notified when object allocated (after #objectOnInitFinish and #ZFClass::instanceObserverAdd)\n
     ...
     * @brief see #ZFObject::observerNotify
     *
     * nofified only if any observer added
     */
     ...
     * nofified only if any observer added
     */
    ZFOBSERVER_EVENT(ObjectAfterRetain)
    /**
     * @brief see #ZFObject::observerNotify
     *
     * nofified only if any observer added
     */
    ZFOBSERVER_EVENT(ObjectBeforeRelease)
    /**
     * @brief see #ZFObject::observerNotify
     *
...
public:
     ...
     * @brief see #observerNotify
     */
    zffinal inline void observerAddWithLevel(ZF_IN ZFLevel level,
                                             ZF_IN const zfidentity &eventId,
                                             ZF_IN const ZFListener &observer,
                                             ZF_IN_OPT ZFObject *userData = zfnull)
    {
        this->observerAdd(eventId, observer, userData, zfnull, zffalse, level);
    }
    /**
     * @brief see #observerNotify
     */
     ...
     * use #ZFGlobalEventCenter for global observer\n
...
protected:
     ...
     * note, for performance, some frequent event won't be fired all the time,
     * such as object property change event,
     * would fired only if any observer added,
     * and this method won't be called if no observer added
     */
...
public:
    ...
    zffinal void _ZFP_ZFObject_objectInstanceStateSet(ZF_IN ZFObject::ObjectInstanceState objectInstanceState);
