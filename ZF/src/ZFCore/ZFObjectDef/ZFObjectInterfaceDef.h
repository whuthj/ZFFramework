...
template<typename T_FromZFObjectOrZFInterface, typename T_ToZFInterface, zfint isInterface>
...
#define _ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_CAST_CALLBACK(Interface, index, total) \
    ...
    { \
        return _ZFP_ZFInterfaceCastWrapper<zfself, Interface, (ZFMACRO_CLASS_IS_TYPE_OF(zfself, ZFObject) ? 0 : 1)>::cast(obj); \
    }
        ...
        { \
            ...
            { \
                cls->_ZFP_ZFClass_interfaceRegister( \
                    ZFMACRO_FIX_PARAM(_ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_CLASS, ZFMACRO_COMMA, ImplementedInterfaces, ##__VA_ARGS__) \
                    , 0 \
                    ); \
                cls->_ZFP_ZFClass_interfaceRegisterCastCallback( \
                    ZFMACRO_FIX_PARAM(_ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_CAST_CALLBACK_NAME, ZFMACRO_COMMA, ImplementedInterfaces, ##__VA_ARGS__) \
                    , 0 \
                    ); \
            } \
        } \
    public: \
        ZFMACRO_FIX_PARAM(_ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_CAST_CALLBACK, ZFMACRO_EMPTY, ImplementedInterfaces, ##__VA_ARGS__) \
    public:
/**
 * @brief see #ZFINTERFACE_DECLARE
 */
#define ZFIMPLEMENTS_DECLARE(ImplementedInterfaces, ...) \
    ...
    public: \
        ...
        { \
            ...
            ZFMACRO_FIX_PARAM(_ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_INTERFACE_ON_INIT, ZFMACRO_EMPTY, ImplementedInterfaces, ##__VA_ARGS__) \
        } \
        virtual inline void _ZFP_ZFObject_interfaceOnDealloc(void) \
        { \
            ZFMACRO_FIX_PARAM(_ZFP_ZFIMPLEMENTS_DECLARE_EXPAND_INTERFACE_ON_DEALLOC, ZFMACRO_EMPTY, ImplementedInterfaces, ##__VA_ARGS__) \
 ...
 *   -  ZFMethod (ZFMETHOD_MEMBER_OVERRIDE_XXX)
