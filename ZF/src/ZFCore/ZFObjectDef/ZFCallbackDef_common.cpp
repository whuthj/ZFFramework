                             ...
                             ZF_IN_OPT ZFSeekPos pos /* = ZFSeekPosBegin */) const
{
    ...
    return method->executeClassMember<zfindex, zfindex, ZFSeekPos>(owner, byteSize, pos);
}
zfindex ZFIOCallback::ioTell(void) const
{
    ...
    return method->executeClassMember<zfindex>(owner);
}
...
ZFMETHOD_FUNCTION_DECLARE_2(zfindex, _ZFP_ZFOutputCallbackForConsoleFunction, const void *, s, zfindex, count)
{
...
{
    this->outputCallbackForConsole = ZFCallbackForMethod(ZFMethodAccessFunction(_ZFP_ZFOutputCallbackForConsoleFunction));
}
...
public:
    zfstring *pString;
    zfindex savedLength;
    ZFMETHOD_MEMBER_DECLARE_2(zfindex, onOutput,
                              const void *, s,
                              zfindex, count)
    {
    ...
    ZFMETHOD_MEMBER_DECLARE_2(zfindex, ioSeek, zfindex, byteSize, ZFSeekPos, pos)
    {
...
public:
    ZFMETHOD_MEMBER_DECLARE_2(zfindex, onInput, void *, buf, zfindex, count)
    {
        if(buf == zfnull)
        {
            return 0;
...
{
    const ZFSerializableData *inputData = ZFSerializableUtil::requireElementByCategory(serializableData, ZFSerializableKeyword_ZFInputCallbackForInputInRange_input, outErrorHintToAppend, outErrorPos);
    if(inputData == zfnull)
    {
        return zffalse;
    }
                                              ...
                                              ZF_IN_OPT zfindex count /* = zfindexMax */)
{
    ...
    owner->p = owner->pStart;
    ZFInputCallback ret = ZFCallbackForMemberMethodDetail(
        owner, ZFMethodAccessClassMember(_ZFP_ZFInputCallbackForBufferOwner, onInput),
        ownerFilePath, ownerFunctionName, ownerFileLine);
    ret.callbackTagSet(ZFCallbackTagKeyword_ioOwner, owner);
    if(copy)
    {
...
public:
    ZFMETHOD_MEMBER_DECLARE_2(zfindex, onInput,
                              void *, buf,
                              zfindex, count)
    {
        if(buf == zfnull)
...
{
    return ZFCallbackForMemberMethod(d, ZFMethodAccessClassMember(_ZFP_ZFIOBridgeCallbackUsingBufferPrivate, onInput));
}
ZFOutputCallback ZFIOBridgeCallbackUsingBuffer::outputCallback(void)
{
    return ZFCallbackForMemberMethod(d, ZFMethodAccessClassMember(_ZFP_ZFIOBridgeCallbackUsingBufferPrivate, onOutput));
}
