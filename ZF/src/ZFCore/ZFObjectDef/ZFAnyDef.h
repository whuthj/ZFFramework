...
private:
    ...
    zfclassNotPOD _ZFP_TypeChecker
    {
    };
    template<typename T_ZFObject>
    zfclassNotPOD _ZFP_TypeChecker<T_ZFObject, 1>
    {
    public:
...
inline ZFAnyT<T_ZFObjectBase>::operator T_ZFObject *(void) const
{
    return ZFCastZFObjectUnchecked(
        typename _ZFP_TypeChecker<T_ZFObject * ZFMACRO_COMMA() ZFMACRO_CLASS_IS_TYPE_OF(typename zftypeTraits<T_ZFObject>::Traits, typename zftypeTraits<T_ZFObjectBase>::Traits)>::TypeMatched,
        this->_obj);
}
template<typename T_ZFObjectBase>
