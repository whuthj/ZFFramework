...
{
    const ZFSerializableData *fileDescriptorData = ZFSerializableUtil::requireElementByCategory(serializableData, ZFSerializableKeyword_ZFInputCallbackForFileDescriptor_fileDescriptor, outErrorHintToAppend, outErrorPos);
...
{
    const ZFSerializableData *fileDescriptorData = ZFSerializableUtil::requireElementByCategory(serializableData, ZFSerializableKeyword_ZFOutputCallbackForFileDescriptor_fileDescriptor, outErrorHintToAppend, outErrorPos);
    if(fileDescriptorData == zfnull)
    {
        return zffalse;
    }
...
ZFCoreArrayPOD<const zfchar *> ZFFileDescriptorTypeGetAll(void)
{
    return _ZFP_ZFFileDescriptorTypeMap.allKey();
}
