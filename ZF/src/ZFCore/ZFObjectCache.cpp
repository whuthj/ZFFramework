...
#include "ZFSTLWrapper/zfstl_list.h"
...
{
    ZFObjectCache *objectCache = listenerData.sender->to<ZFObjectCache *>();
    objectCache->cacheTrim();
}
void cacheTrimListenerSetup(ZF_IN ZFObjectCache *objectCache)
{
    if(objectCache->cacheTrimWhenReceiveMemoryWarning())
    {
        ZFGlobalEventCenter::instance()->observerAdd(
            ZFGlobalEvent::EventAppOnReceiveMemoryWarning(),
            this->cacheTrimListener,
            objectCache->objectHolder(),
            objectCache);
    }
    else
    {
        ZFGlobalEventCenter::instance()->observerRemoveByOwner(objectCache);
    }
}
...
ZFPROPERTY_CUSTOM_SETTER_DEFINE(ZFObjectCache, zfbool, cacheTrimWhenReceiveMemoryWarning)
{
    ...
    ZF_GLOBAL_INITIALIZER_INSTANCE(ZFObjectCacheDataHolder)->cacheTrimListenerSetup(this);
}
