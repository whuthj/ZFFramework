...
_ZFP_ZFVALUE_DUMMY_FLAG_TO_ADD_TYPE
#define _ZFP_ZFValue_initValue_bool() zffalse
#define _ZFP_ZFValue_initValue_char() '\0'
#define _ZFP_ZFValue_initValue_int() 0
#define _ZFP_ZFValue_initValue_unsignedInt() 0
#define _ZFP_ZFValue_initValue_index() 0
#define _ZFP_ZFValue_initValue_int8() 0
#define _ZFP_ZFValue_initValue_unsignedInt8() 0
#define _ZFP_ZFValue_initValue_int16() 0
#define _ZFP_ZFValue_initValue_unsignedInt16() 0
#define _ZFP_ZFValue_initValue_int32() 0
#define _ZFP_ZFValue_initValue_unsignedInt32() 0
#define _ZFP_ZFValue_initValue_int64() 0
#define _ZFP_ZFValue_initValue_unsignedInt64() 0
#define _ZFP_ZFValue_initValue_float() 0
#define _ZFP_ZFValue_initValue_double() 0
#define _ZFP_ZFValue_initValue_longDouble() 0
#define _ZFP_ZFValue_initValue_time() 0
#define _ZFP_ZFValue_initValue_flags() 0
#define _ZFP_ZFValue_initValue_identity() 0
                                                ...
                                                ZF_OUT_OPT const ZFSerializableData **outErrorPos /* = zfnull */)
{
    ...
    const zfchar *valueTypeString = ZFSerializableUtil::checkAttribute(serializableData, ZFSerializableKeyword_ZFValue_valueType);
    const zfchar *valueString = ZFSerializableUtil::checkAttribute(serializableData, ZFSerializableKeyword_ZFValue_value);
    if(valueTypeString == zfnull && valueString == zfnull)
    {
        this->intValueSet(0);
        return zftrue;
    }
    ...
    // valueType
    ZFValueTypeEnum valueType = ZFValueType::e_int;
    if(valueTypeString != zfnull && ZFValueTypeEnumFromString(valueType, valueTypeString) != zfnull)
    {
        ZFSerializableUtil::errorOccurredWhile(outErrorHintToAppend, outErrorPos, serializableData, ZFSerializableKeyword_ZFValue_valueType, valueTypeString);
        return zffalse;
    }
    ...
    switch(valueType)
    {
        #define _ZFP_ZFValue_serializableOnSerializeFromData(TypeName, Type) \
            case ZFValueType::ZFMACRO_CAT(e_, TypeName): \
            { \
                Type v = _ZFP_ZFValue_initValue(TypeName); \
                if(valueString != zfnull && Type##FromString(v, valueString) != zfnull) \
                { \
                    ZFSerializableUtil::errorOccurredWhile(outErrorHintToAppend, outErrorPos, serializableData, ZFSerializableKeyword_ZFValue_value, valueString); \
                    return zffalse; \
                } \
                                              ...
                                              ZF_OUT_OPT zfstring *outErrorHintToAppend /* = zfnull */)
{
    ...
    if(ref != zfnull)
    {
        if(ref->objectCompare(this) == ZFCompareTheSame)
        {
            return zftrue;
        }
    }
    ...
    { // valueType
        ...
        if(!ZFValueTypeEnumToSerializableData(categoryData, this->valueType(), outErrorHintToAppend))
        {
            return zffalse;
        }
        categoryData.categorySet(ZFSerializableKeyword_ZFValue_valueType);

        zfbool needAdd = zftrue;
        if(ref != zfnull)
        {
            if(ref->valueType() == this->valueType())
            {
                needAdd = zffalse;
            }
        }
        if(needAdd)
        {
            const ZFSerializableData *refInfo = this->serializableRefInfoGetForCategory(ZFSerializableKeyword_ZFValue_valueType);
            if(refInfo != zfnull)
            {
            ...
            else if(this->valueType() == ZFValueType::e_int)
            {
                needAdd = zffalse;
            }
        }
    ...
    { // value
        ...
        switch(this->valueType())
        {
            #define _ZFP_ZFValue_serializableOnSerializeToData(TypeName, Type) \
                case ZFValueType::ZFMACRO_CAT(e_, TypeName): \
                { \
                    if(!Type##ToSerializableData(categoryData, this->TypeName##ValueAccess())) \
                    { \
                        return zffalse; \
                    } \
                    categoryData.categorySet(ZFSerializableKeyword_ZFValue_value); \
                    if(ref != zfnull) \
                    { \
                        if(ref->valueType() == this->valueType() \
                            && ref->TypeName##ValueAccess() == this->TypeName##ValueAccess()) \
                        { \
                            needAdd = zffalse; \
                        } \
                    } \
                    if(needAdd) \
                    { \
                        const ZFSerializableData *refInfo = this->serializableRefInfoGetForCategory(ZFSerializableKeyword_ZFValue_value); \
                        if(refInfo != zfnull) \
                        { \
                            ZFSerializableData tmpData; \
                            if(!refInfo->referenceLoadTo(tmpData, outErrorHintToAppend)) \
                        ...
                        else if(this->TypeName##ValueAccess() == _ZFP_ZFValue_initValue(TypeName)) \
                        { \
                            needAdd = zffalse; \
                        } \
                    } \
            ...
            case ZFValueType::e_serializableData:
            {
                const ZFSerializableData &serializableValue = this->serializableDataValueAccess();
                if(!serializableValue.isEmpty())
                {
                    zfstring tmp;
                    if(!ZFSerializableDataToString(tmp, serializableValue, outErrorHintToAppend))
                    {
                        return zffalse;
                    }
                    if(!zfstringToSerializableData(categoryData, tmp, outErrorHintToAppend))
                    {
                        return zffalse;
                    }
                    categoryData.categorySet(ZFSerializableKeyword_ZFValue_value);
...
#define _ZFP_ZFValue_getValue_DEFINE(TypeName, Type) \
    ...
    { \
        ...
        { \
            ...
            case ZFValueType::e_serializableData: \
                _ZFP_ZFValueLogUnConvertable(this->valueType(), ZFValueType::ZFMACRO_CAT(e_, TypeName)); \
...
zfbool ZFValue::boolValue(void)
{
    switch(this->valueType())
    {
        ...
        case ZFValueType::e_serializableData:
            _ZFP_ZFValueLogUnConvertable(this->valueType(), ZFValueType::ZFMACRO_CAT(e_, bool));
            return zffalse;
        default:
            zfCoreCriticalShouldNotGoHere();
            return zffalse;
...
#define _ZFP_ZFValue_setValue_DEFINE(TypeName, Type) \
    ...
    { \
        d->clearValue(); \
        d->type = ZFValueType::ZFMACRO_CAT(e_, TypeName); \
        d->v.v_##TypeName = v; \
    }
