...
static _ZFP_ZFStringPrivate *_ZFP_ZFString_readyUpdate(_ZFP_ZFStringPrivate *&old)
{
    if(old == zfnull)
    {
                                                 ...
                                                 ZF_OUT_OPT const ZFSerializableData **outErrorPos /* = zfnull */)
{
    ...
    this->stringValueSet(ZFSerializableUtil::checkPropertyValue(serializableData));
    return zftrue;
}
                                               ...
                                               ZF_OUT_OPT zfstring *outErrorHintToAppend /* = zfnull */)
{
    ...
    if(ref != zfnull)
    {
        needAdd = !zfscmpTheSame(this->stringValue(), ref->stringValue());
    }
    else
    {
        needAdd = !zfscmpTheSame(this->stringValue(), zfText(""));
    }
    if(needAdd)
    {
        serializableData.propertyValueSet(this->stringValue());
    }
    return zftrue;
}
