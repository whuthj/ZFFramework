...
public:
    ZFOperationAsync *ownerOperation;
    ZFCoreArrayPOD<zfidentity> runningOperationId;
    ZFCoreArrayPOD<zfidentity> runningThreads;

public:
    ZFLISTENER_MEMBER_DECLARE(onNotifyFinishInMainThread)
    {
        this->ownerOperation->_ZFP_ZFOperationAsync_taskNotifyFinishAsync(
    ...
    ZFLISTENER_MEMBER_DECLARE(onNotifyFinishInMainThreadWithOperationId)
    {
        this->ownerOperation->_ZFP_ZFOperationAsync_taskNotifyFinishAsync(
            ZFCastStatic(zfidentity, ZFCastZFObjectUnchecked(ZFValue *, listenerData.param0)->indexValue()),
            ZFCastZFObjectUnchecked(ZFOperationResult *, listenerData.param1));
    }
    ZFLISTENER_MEMBER_DECLARE(onNotifyProgressInMainThread)
    {
        this->ownerOperation->_ZFP_ZFOperationAsync_taskNotifyProgressAsync(
            ZFCastZFObjectUnchecked(ZFOperationParam *, listenerData.param0),
            ZFCastZFObjectUnchecked(ZFOperationProgress *, listenerData.param1));
    }
    ZFLISTENER_MEMBER_DECLARE(onNotifyProgressInMainThreadWithOperationId)
    {
        this->ownerOperation->_ZFP_ZFOperationAsync_taskNotifyProgressAsync(
            ZFCastStatic(zfidentity, ZFCastZFObjectUnchecked(ZFValue *, listenerData.param0)->indexValue()),
            ZFCastZFObjectUnchecked(ZFOperationProgress *, listenerData.param1));
    }
    ZFLISTENER_MEMBER_DECLARE(onStartInNewThread)
    {
        this->ownerOperation->_ZFP_ZFOperationAsync_taskOnStartAsync(ZFCastZFObjectUnchecked(ZFOperationTaskData *, userData));
    }
};
