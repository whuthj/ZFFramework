...
ZF_GLOBAL_INITIALIZER_INIT_WITH_LEVEL(zfSkinApplyDataHolder, ZFLevelZFFrameworkLow)
{
    ...
    ZFGlobalEventCenter::instance()->observerAdd(ZFGlobalEvent::EventSkinOnChange(), this->skinOnChangeListener);
}
ZF_GLOBAL_INITIALIZER_DESTROY(zfSkinApplyDataHolder)
{
    ZFGlobalEventCenter::instance()->observerRemove(ZFGlobalEvent::EventSkinOnChange(), this->skinOnChangeListener);

    if(!this->dataMap.empty())
    {
        _ZFP_zfSkinApplyDataMapType tmp = this->dataMap;
        this->dataMap.clear();
