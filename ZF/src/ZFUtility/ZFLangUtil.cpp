...
ZF_GLOBAL_INITIALIZER_INIT_WITH_LEVEL(zfLangApplyDataHolder, ZFLevelZFFrameworkLow)
{
    ...
    ZFGlobalEventCenter::instance()->observerAdd(ZFGlobalEvent::EventLangOnChange(), this->langOnChangeListener);
}
ZF_GLOBAL_INITIALIZER_DESTROY(zfLangApplyDataHolder)
{
    ZFGlobalEventCenter::instance()->observerRemove(ZFGlobalEvent::EventLangOnChange(), this->langOnChangeListener);

    if(!this->dataMap.empty())
    {
        _ZFP_zfLangApplyDataMapType tmp = this->dataMap;
        this->dataMap.clear();
...
public:
                            ...
                            ZF_IN_OPT ZFObject *userData)
    {
        userData->to<ZFPointerHolder *>()->holdedDataPointer<const ZFProperty *>()
            ->setterMethod()->executeClassMember<void, const zfstring &>(obj, langValue);
    }
};
