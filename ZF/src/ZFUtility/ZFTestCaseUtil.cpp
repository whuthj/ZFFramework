...
public:
    ZFCoreArrayPOD<const ZFClass *> testCaseList(void)
    {
        this->testCaseListPrepare();
        ZFCoreArrayPOD<const ZFClass *> tmp;
        tmp.copyFrom(this->testCases);
        return tmp;
    }
...
ZFCoreArrayPOD<const ZFClass *> ZFTestCaseGetAll(void)
{
    return ZF_GLOBAL_INITIALIZER_INSTANCE(ZFTestCaseRunAllHolder)->testCaseList();
}
