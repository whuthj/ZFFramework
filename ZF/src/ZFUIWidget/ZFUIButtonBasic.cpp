...
{
public:
    ZFUIButtonBasic *owner;
    ZFUITextViewStyle *buttonLabel;
    ZFUIImageViewStyle *buttonIcon;
    ZFUIImageViewStyle *buttonBackground;
...
public:
    ...
    {
        ZFString *value = this->owner->buttonLabelStyle(this->owner->buttonState())->textContent();
        if(value == zfnull)
        {
            if(this->owner->buttonState() == ZFUIButtonState::e_Checked)
            {
                value = this->owner->buttonLabelStyleHighlighted()->textContent();
            }
            else if(this->owner->buttonState() == ZFUIButtonState::e_CheckedHighlighted)
            {
                value = this->owner->buttonLabelStyleChecked()->textContent();
                if(value == zfnull)
                {
                    value = this->owner->buttonLabelStyleHighlighted()->textContent();
                }
            ...
            if(value == zfnull)
            {
                value = this->owner->buttonLabelStyleNormal()->textContent();
            }
        }
    ...
    {
        ZFUIImage *value = this->owner->buttonIconStyle(this->owner->buttonState())->imageContent();
        if(value == zfnull)
        {
            if(this->owner->buttonState() == ZFUIButtonState::e_Checked)
            {
                value = this->owner->buttonIconStyleHighlighted()->imageContent();
            }
            else if(this->owner->buttonState() == ZFUIButtonState::e_CheckedHighlighted)
            {
                value = this->owner->buttonIconStyleChecked()->imageContent();
                if(value == zfnull)
                {
                    value = this->owner->buttonIconStyleHighlighted()->imageContent();
                }
            ...
            if(value == zfnull)
            {
                value = this->owner->buttonIconStyleNormal()->imageContent();
            }
        }
    ...
    {
        ZFUIImage *value = this->owner->buttonBackgroundStyle(this->owner->buttonState())->imageContent();
        if(value == zfnull)
        {
            if(this->owner->buttonState() == ZFUIButtonState::e_Checked)
            {
                value = this->owner->buttonBackgroundStyleHighlighted()->imageContent();
            }
            else if(this->owner->buttonState() == ZFUIButtonState::e_CheckedHighlighted)
            {
                value = this->owner->buttonBackgroundStyleChecked()->imageContent();
                if(value == zfnull)
                {
                    value = this->owner->buttonBackgroundStyleHighlighted()->imageContent();
                }
            ...
            if(value == zfnull)
            {
                value = this->owner->buttonBackgroundStyleNormal()->imageContent();
            }
        }
...
public:
    #define _ZFP_ZFUIButtonBasic_LISTENER_EXPAND_STYLE_CHANGED(T_Component, T_State) \
        ...
        { \
            ...
            else if(property == ZFPropertyAccess(ZFUITextViewStyle, textContent)) \
            { \
...
ZFSTYLE_DEFAULT_DEFINE(ZFUIButtonBasicStyle, ZFUIButtonStyle)
...
void ZFUIButtonBasic::objectOnDealloc(void)
{
    ...
    #define _ZFP_ZFUIButtonBasic_componentObserverRemove(Component, State) \
        property = ZFPropertyAccess(ZFUIButtonBasicStyle, button##Component##Style##State); \
                                      ...
                                      ZF_IN const ZFUISizeParam &sizeParam)
{
    ...
    if(labelView != zfnull && labelView->viewVisible() && d->buttonLabel->textContent() != zfnull)
    {
    ...
    ret = ZFUISizeMake(
        zfmMax(backgroundSize.width + ZFUIMarginGetX(this->buttonBackgroundMargin()),
...
ZFUITextViewStyle *ZFUIButtonBasic::buttonLabel(void)
{
    this->prepareButtonLabel();
    return d->buttonLabel;
}
ZFUIImageViewStyle *ZFUIButtonBasic::buttonIcon(void)
{
    this->prepareButtonIcon();
    return d->buttonIcon;
}
ZFUIImageViewStyle *ZFUIButtonBasic::buttonBackground(void)
{
...
void ZFUIButtonBasic::objectPropertyValueOnInit(ZF_IN const ZFProperty *property)
{
    ...
    #define _ZFP_ZFUIButtonBasic_objectPropertyValueOnInit(T_Component, T_State) \
        else if(property == ZFPropertyAccess(ZFUIButtonBasicStyle, button##T_Component##Style##T_State)) \
        { \
            this->prepareButton##T_Component(); \
            this->button##T_Component##Style##T_State()->toObject()->observerAdd(ZFObject::EventObjectPropertyValueAfterChange(), ZF_GLOBAL_INITIALIZER_INSTANCE(ZFUIButtonBasicListenerHolder)->button##T_Component##Style##T_State##ChangedListener, this->objectHolder()); \
        }
