...
@synthesize _ZFP_ownerZFUIWebView, _ZFP_impl;
- (instancetype)init
{
    self = [super init];
    if(self)
...
ZFPROTOCOL_IMPLEMENTATION_BEGIN(ZFUIWebViewImpl_sys_iOS, ZFUIWebView, ZFProtocolLevelSystemNormal)
...
public:
    ...
    {
        _ZFP_ZFUIWebViewImpl_sys_iOS_View *nativeWebViewTmp = ZFCastStatic(_ZFP_ZFUIWebViewImpl_sys_iOS_View *, nativeWebView);
        // load empty data to reduce memory usage
        [nativeWebViewTmp loadHTMLString:@"" baseURL:nil];
        [nativeWebViewTmp release];
    }

    virtual void webLoadUrl(ZF_IN ZFUIWebView *webView,
                            ZF_IN const zfchar *url)
    {
        _ZFP_ZFUIWebViewImpl_sys_iOS_View *nativeWebView = ZFCastStatic(_ZFP_ZFUIWebViewImpl_sys_iOS_View *, webView->nativeImplView());
